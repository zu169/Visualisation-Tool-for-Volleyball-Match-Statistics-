...
const addPoint = (team: "home" | "away", index: number) => {
  ...
  if (homeVisible && awayVisible) {
    ...
    if (team === "home") {
      const count = awayButtonsVisible.value.filter(Boolean).length;
      if (count > awayPoints.value)
        awayButtonsVisible.value.pop(),
          homeButtonsVisible.value.pop(),
          homeButtonsVisible.value.push(true);
    } else {
      const count = homeButtonsVisible.value.filter(Boolean).length;
      if (count > homePoints.value)
        homeButtonsVisible.value.pop(),
          awayButtonsVisible.value.pop(),
          awayButtonsVisible.value.push(true);
    }
  }
};
</script>
...
<template>
  ...
  <div class="flex w-full">
    ...
    <div class="flex flex-col gap-2 w-1/2 items-end">
      <template
        v-for="(visible, index) in homeButtonsVisible"
        :key="'home' + index"
      >
        <UButton
          v-if="visible"
          ...
          >{{ index + 1 }}</UButton
        >
        <div v-else class="w-10 h-10 invisible" />
      </template>
    </div>
    ...
    <div class="flex flex-col gap-2 w-1/2 items-start">
      <template
        v-for="(visible, index) in awayButtonsVisible"
        :key="'away' + index"
      >
        <UButton
          v-if="visible"
